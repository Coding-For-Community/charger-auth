...
export function useUserToken(kioskToken: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetchBackend("/checkin/userToken/?kiosk_token=" + kioskToken)
      if (!res.ok) {
...
export async function checkIn(
  ...
  const body = JSON.stringify({ email_b64, mode, device_id, user_token })
...
export async function parseCheckInRes(res: Response): Promise<CheckInResult> {
  switch (res.status) {
    ...
    case 400:
      return { status: "err", msg: "Invalid Student ID/Email - maybe close and re-open ChargerAuth?" }
    case 401:
    ...
    case 416:
      return { status: "err", msg: "You're trying to check back in for senior privileges, but you haven't checked out yet." }
    default:
