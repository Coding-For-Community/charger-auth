...
import { EMAIL_KEY } from "../utils/constants.ts";
import { load as loadAgent } from '@fingerprintjs/fingerprintjs';
import { enablePushNotifs } from "../utils/enablePushNotifs.ts";
...
function MobileApp() {
  ...
  const deviceIdQ = useDeviceIdQuery()
  ...
  if (tokenQuery.isSuccess && deviceIdQ.isSuccess) {
    return (
      <AppShell>
          ...
          <Stack style={{
            ...
            <QRCodeSVG 
              value={JSON.stringify({
                email_b64: emailB64, 
                checkin_token: tokenQuery.data["token"],
                device_id: deviceIdQ.data
              })} 
              size={350}
            />
          </Stack>
